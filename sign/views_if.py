...
def add_event(request):
    ...
    if result:
        return JsonResponse({'statue': 10023, 'message': 'event name already exists'})
...
def get_event_list(request):
    ...
    if eid == '' and name == '':
        return JsonResponse({'statue': 10021, 'message': 'parameter error'})
...
def add_guest(request):
    ...
    try:
        Guest.objects.create(real_name=real_name, phone=lnt(phone), email=email, sign=0, event_id=int(eid))
